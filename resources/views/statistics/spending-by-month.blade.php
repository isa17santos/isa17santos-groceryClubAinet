...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('statistics.member-spending-by-month.export') }}"
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="text-lg font-semibold text-gray-700 dark:text-gray-300 border-b">
                    ...
                    <th class="pb-3 text-right">Total Spending</th>
                </tr>
            </thead>
            <tbody class="text-gray-700 dark:text-gray-200">
                @foreach ($spendingByMonth as $entry)
                    <tr class="border-b border-gray-200 dark:border-gray-600">
                        <td class="py-4">{{ \Carbon\Carbon::parse($entry->month . '-01')->format('F Y') }}</td>
                        <td class="py-4 text-right font-semibold">€{{ number_format($entry->total_spent, 2, ',', '.') }}</td>
                    </tr>
                @endforeach
                <tr class="border-t font-bold text-gray-800 dark:text-gray-100">
                    <td class="py-4">Total</td>
                    <td class="py-4 text-right font-semibold">
                        €{{ number_format($spendingByMonth->sum('total_spent'), 2, ',', '.') }}
                    </td>
                </tr>
            </tbody>
        </table>
