...
@section('content')
@if ($errors->any())
    <div class="mb-4 p-3 bg-red-100 dark:bg-red-200 text-red-700 rounded-md shadow-sm max-w-2xl mx-auto">
...
<div class="max-w-2xl mx-auto mt-16 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow">
    ...
    @php
        $defaultPhoto = asset('storage/users/' . ($user->photo ?? 'anonymous.png'));
    @endphp

    <form method="POST" action="{{ route('board.users.update', $user) }}" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div x-data="{ photoPreview: '{{ $defaultPhoto }}' }" class="flex flex-col items-start">
                ...
                <input type="file" name="photo" accept="image/*"
                    ...
                    class="w-full p-2 rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
            </div>
        </div>
